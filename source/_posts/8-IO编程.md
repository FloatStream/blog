...
3. **参数**: `filename`--包含了需要访问的文件名称的字符串; `mode`--打开文件的模式(详解下表); `buffering`--设置缓冲; `encoding`--编码方式; `errors`--报错级别; `newline`--区分换行符;`closefd`--传入的**file**参数类型; `opener`--设置自定义开启器, 开启器的返回值必须是一个打开的文件描述符
...
该内容主要使用了 `os` 模块, 可以作为 [os 模块](4_模块详解#6%20OS模块)  的详细介绍 
...
Python提供了`pickle`模块来实现序列化, 参考[pickle模块](4_模块详解.md#5%20pickle模块)
